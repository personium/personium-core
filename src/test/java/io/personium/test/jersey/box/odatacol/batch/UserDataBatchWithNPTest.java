...
import static io.personium.test.utils.BatchUtils.retrieveListBody;
import static io.personium.test.utils.BatchUtils.retrievePostBody;
import static io.personium.test.utils.BatchUtils.retrievePostBodyOfProperty;
import static io.personium.test.utils.BatchUtils.retrievePostNoneBody;
...
import static io.personium.test.utils.BatchUtils.retrieveQueryOperationResErrorBody;
...
public class UserDataBatchWithNPTest extends AbstractUserDataBatchTest {
    ...
    public final void $batchで既存のユーザデータからNavPro経由で登録済みの__idを指定した場合ユーザデータが登録できないこと() {
        try {
            ...
            // $batch前のデータ取得
            TResponse original = UserDataUtils.get(cellName, PersoniumUnitConfig.getMasterToken(), boxName, colName,
            ...
            // ユーザデータ取得 書き換わっていない
            TResponse after = UserDataUtils.get(cellName, PersoniumUnitConfig.getMasterToken(), boxName, colName,
                    "Product", "id0001", HttpStatus.SC_OK);
    ...
    public final void $batchで既存のユーザデータからNavPro経由で最大プロパティ数を超えたユーザデータが登録できないこと() {
        try {
            ...
            int maxPropNum = PersoniumUnitConfig.getMaxPropertyCountInEntityType() + 1;
            super.createUserData(srcBody, HttpStatus.SC_CREATED, cellName, boxName, colName, "Sales");
            String path = "Sales('srcKey')/_Product";
            String body = START_BOUNDARY
                    + retrievePostBodyOfProperty(path, "id0001", maxPropNum)
    ...
    public final void $batchのNavPro経由ユーザデータ登録で登録データの__idにnullが指定された場合登録できること() {
        try {
            ...
            ResourceUtils.deleteUserDataLinks("srcKey", "null", "Sales", cellName, boxName, colName,
                    "Product", -1);
            deleteUserData(cellName, boxName, colName, "Sales", "null", PersoniumUnitConfig.getMasterToken(), -1);
            deleteUserData(cellName, boxName, colName, "Product", "srcKey", PersoniumUnitConfig.getMasterToken(), -1);
        }
    }
